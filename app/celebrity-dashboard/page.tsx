...
export default function CelebrityDashboard() {
  ...
  const handleBookingAction = async (requestId: string, action: "accept" | "decline") => {
    ...
    try {
      ...
      // Show success toast
      toast.success(`Booking ${action === 'accept' ? 'accepted' : 'declined'} successfully!`)
    } catch (error) {
  ...
  const exportPaymentsPDF = async () => {
    try {
      ...
      Object.entries(monthlyData).forEach(([month, data]: [string, any]) => {
        pdf.text(`${month}: $${data.total.toFixed(2)} (${data.count} orders)`, 20, yPos)
        yPos += 8
      })
  ...
  const exportTipsPDF = async () => {
    try {
      ...
      let yPos = 100
      Object.entries(monthlyData).forEach(([month, data]: [string, any]) => {
        pdf.text(`${month}: $${data.total.toFixed(2)} (${data.count} tips)`, 20, yPos)
        yPos += 8
      })
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
                  ...
                  <Card className="bg-white/10 border-white/20 backdrop-blur-lg">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-purple-200 text-sm font-medium">Available in Stripe</p>
                          <p className="text-2xl font-bold text-blue-400">
                            ${(stats.stripeAvailableBalance || 0).toLocaleString()}
                          </p>
                          <p className="text-xs text-blue-400 mt-1">Ready for payout</p>
                        </div>
                                ...
                                <div>
                                 ...
                                 <div className="flex flex-col gap-1">
                                   ...
                                   <Badge className="bg-green-500/20 text-green-300 text-xs">
                                     ${((request.celebrityAmount || 0) / 100).toLocaleString()}
                                   </Badge>
                                   {request.tipAmount > 0 && (
                                     <Badge className="bg-yellow-500/20 text-yellow-300 text-xs">
                                       +${(request.tipAmount / 100).toLocaleString()} tip
                                     </Badge>
                                   )}
                                 </div>
                                 <p className="text-xs text-purple-400 mt-1">
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-purple-200 text-sm">Total tips received</p>
                    </div>
                                         <div className="flex gap-2">
                       <Button
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-purple-200 text-sm">Total payments received</p>
                    </div>
                                         <div className="flex gap-2">
                       <Button
                  ...
                  {requestsLoading ? (
                    <div className="flex justify-center py-8">
                     ...
                     <div className="text-center py-8">
                       ...
                       <p className="text-white font-semibold mb-2">No recent requests</p>
                                ...
                                {request.tips.map((tip) => (
                                  ...
                                  >
                                    ...
                                    {tip.message && <p className="text-white text-sm mt-2">{tip.message}</p>}
                                                     ...
                                                     {request.approvalStatus?.toLowerCase() === "pending_approval" && (
                      ...
                      >
                        ...
                        <div className="flex gap-3">
                          ...
                          <Button
                            variant="outline"
                            className="bg-white/10 border-white/20 text-white hover:bg-white/20"
                            onClick={() => router.push(`/celebrity/bookings/${order.id}`)}
                          >
