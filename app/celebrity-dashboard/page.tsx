...
import { CelebrityProfileImageUpload } from "@/components/celebrity-profile-image-upload";
...
export default function CelebrityDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "❌ Celebrity Dashboard - Stats fetch failed:",
          errorText
        );
        throw new Error(
      ...
      setStats({
        ...
        stripeAvailableBalance: data.stripeAvailableBalance || 0,
      });
    } catch (error) {
  ...
  const fetchBookingRequests = async () => {
    try {
      ...
      const response = await fetch(
        "/api/celebrity/booking-requests?status=PENDING&limit=10"
      );
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "❌ Celebrity Dashboard - Booking requests fetch failed:",
          errorText
        );
        throw new Error(
          `Failed to fetch booking requests: ${response.status} ${errorText}`
        );
      }
      const data = await response.json();
      ...
      console.error(
        "❌ Celebrity Dashboard - Error fetching booking requests:",
        error
      );
      setBookingRequests([]);
    } finally {
  ...
  const fetchAllOrders = async () => {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "❌ Celebrity Dashboard - All orders fetch failed:",
          errorText
        );
        throw new Error(
          `Failed to fetch all orders: ${response.status} ${errorText}`
        );
      }
      const data = await response.json();
      ...
      console.log(
        "🔍 Declined orders in transformed data:",
        transformedOrders.filter(
          (order: any) => order.approvalStatus === "DECLINED"
        )
      );
      ...
      console.error(
        "❌ Celebrity Dashboard - Error fetching all orders:",
        error
      );
      setAllOrders([]);
    } finally {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "❌ Celebrity Dashboard - Profile fetch failed:",
          errorText
        );
        throw new Error(
  ...
  const handleBookingAction = async (
    ...
    try {
      console.log(
        `📡 Making API call to: /api/celebrity/booking-requests/${requestId}`
      );

      const response = await fetch(
        `/api/celebrity/booking-requests/${requestId}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          `❌ Celebrity Dashboard - Failed to ${action} booking:`,
          errorText
        );
        throw new Error(
          `Failed to update booking: ${response.status} ${errorText}`
        );
      }
      ...
      // Update local state immediately with correct status mapping
        ...
        prev.map((request) =>
          request.id === requestId
            ? {
                ...
                approvalStatus: action === "accept" ? "APPROVED" : "DECLINED",
              }
            : request
        )
      );
      ...
      console.error(
        `❌ Celebrity Dashboard - Error ${action}ing booking:`,
        error
      );
      // Show error toast
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "❌ Celebrity Dashboard - Profile save failed:",
          errorText
        );
        throw new Error(
    ...
    switch (platform) {
      ...
      case "twitter":
        shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(
          ...
          shareText
        )}&hashtags=KiaOra,Celebrity,PersonalizedMessage`;
        break;
      case "linkedin":
        shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(
          profileUrl
        )}&title=${encodeURIComponent(
          `${profile.name} on Kia Ora`
  ...
  const exportTipsReport = async () => {
    try {
      ...
      a.href = url;
      a.download = `tips-report-${format(new Date(), "yyyy-MM-dd")}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      toast.success("Tips Report Exported!", {
        description: "Your tips report has been downloaded.",
      });
    } catch (error) {
      console.error("Export error:", error);
      toast.error("Export Failed!", {
        description: "Failed to export tips report. Please try again.",
      });
    }
  };
  ...
  const exportPaymentsReport = async () => {
    try {
      const response = await fetch("/api/celebrity/earnings?export=true");
      ...
      toast.success("Payments Report Exported!", {
        description: "Your payments report has been downloaded.",
      });
    } catch (error) {
      console.error("Export error:", error);
      toast.error("Export Failed!", {
  ...
  // PDF Export functions with charts
  const exportPaymentsPDF = async () => {
    try {
      // Dynamically import jsPDF and Chart.js
      const [{ default: jsPDF }, { Chart, registerables }] = await Promise.all([
        import("jspdf"),
        import("chart.js"),
      ]);
      ...
      const data = await response.json();
      // Use recentPayouts and pendingOrders from the API response
      const recentPayouts = data.recentPayouts || [];
      const pendingOrders = data.pendingOrders || [];
      const allPayments = [...recentPayouts, ...pendingOrders];
      ...
      const monthlyData = allPayments.reduce((acc: any, payment: any) => {
        ...
        // Use amount for recentPayouts, expectedPayout for pendingOrders
        const amount = payment.amount || payment.expectedPayout || 0;
        acc[month].total += amount / 100;
        acc[month].count += 1;
        return acc;
      }, {});
      ...
      // Title
      pdf.setFontSize(20);
      pdf.text("Payments Report", 20, 20);
      pdf.setFontSize(12);
      pdf.text(`Generated on ${format(new Date(), "MMM dd, yyyy")}`, 20, 30);

      // Summary
      const totalEarnings = data.summary?.totalEarnings || 0;
      const totalPending = data.summary?.pendingEarnings || 0;
      const totalOrders = recentPayouts.length + pendingOrders.length;

      pdf.setFontSize(14);
      pdf.text("Summary", 20, 50);
      pdf.setFontSize(12);
      pdf.text(`Total Earnings: $${(totalEarnings / 100).toFixed(2)}`, 20, 60);
      pdf.text(`Pending Earnings: $${(totalPending / 100).toFixed(2)}`, 20, 70);
      pdf.text(`Total Orders: ${totalOrders}`, 20, 80);

      // Monthly breakdown
      pdf.setFontSize(14);
      pdf.text("Monthly Breakdown", 20, 90);
      pdf.setFontSize(10);

      let yPos = 100;
      Object.entries(monthlyData).forEach(([month, data]: [string, any]) => {
        pdf.text(
          `${month}: $${data.total.toFixed(2)} (${data.count} orders)`,
          20,
          yPos
        );
        yPos += 8;
      ...
      if (ctx) {
        ...
        pdf.addImage(chartImage, "PNG", 20, yPos + 10, 80, 60);
      }
      ...
      toast.success("Payments PDF Exported!", {
        description: "Your payments report with charts has been downloaded.",
      });
    } catch (error) {
      console.error("PDF Export error:", error);
      toast.error("PDF Export Failed!", {
        description: "Failed to export payments PDF. Please try again.",
      });
    }
  };

  const exportTipsPDF = async () => {
    try {
      // Dynamically import jsPDF and Chart.js
      const [{ default: jsPDF }, { Chart, registerables }] = await Promise.all([
        import("jspdf"),
      ...
      // Group tips by month
      const monthlyData = tips.reduce((acc: any, tip: any) => {
        const month = format(new Date(tip.createdAt), "MMM yyyy");
        if (!acc[month]) {
          acc[month] = { total: 0, count: 0 };
        }
        acc[month].total += tip.amount / 100;
        acc[month].count += 1;
        return acc;
      }, {});
      ...
      Object.entries(monthlyData).forEach(([month, data]: [string, any]) => {
        pdf.text(
          `${month}: $${data.total.toFixed(2)} (${data.count} tips)`,
          20,
          yPos
        );
        yPos += 8;
      ...
      // Download PDF
      pdf.save(`tips-report-${format(new Date(), "yyyy-MM-dd")}.pdf`);

      toast.success("Tips PDF Exported!", {
        description: "Your tips report with charts has been downloaded.",
      });
    } catch (error) {
      console.error("PDF Export error:", error);
      toast.error("PDF Export Failed!", {
        description: "Failed to export tips PDF. Please try again.",
      });
    }
  };
  ...
  const getFilteredOrders = () => {
    ...
    if (orderFilter === "pending_approval")
      return allOrders.filter(
        (order) => order.approvalStatus === "PENDING_APPROVAL"
      );
    if (orderFilter === "approved")
      return allOrders.filter((order) => order.approvalStatus === "APPROVED");
    if (orderFilter === "declined")
      return allOrders.filter((order) => order.approvalStatus === "DECLINED");
    return allOrders;
  };
  ...
  const getDeclinedOrders = (): BookingRequest[] => {
    ...
    return allOrders.filter(
      (order) =>
        order.approvalStatus === "DECLINED" && (order.revisionCount || 0) < 2 // Assuming max 2 revisions
    );
  };
      ...
      <div className="min-h-screen bg-black flex items-center justify-center">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-purple-200">
            You need to be signed in to access the celebrity dashboard.
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-black flex items-center justify-center">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-purple-200 mb-6">
            This dashboard is only available to verified celebrities.
          </p>
                  ...
                  <Card className="bg-gradient-to-r from-red-500/20 to-orange-500/20 border-red-500/30 backdrop-blur-lg">
                    ...
                    <CardContent className="space-y-4">
                      <p className="text-red-200 text-sm mb-4">
                        These customers have requested changes to their videos.
                        Please upload revised videos as soon as possible.
                      </p>
                      {getDeclinedOrders()
                        .slice(0, 3)
                        .map((order) => (
                      ...
                      {getDeclinedOrders().length > 3 && (
                        ...
                        >
                          View All {getDeclinedOrders().length} Revision
                          Requests
                        </Button>
                      )}
                    </CardContent>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-green-400">
                                ${(stats.orderEarnings || 0).toLocaleString()}{" "}
                                bookings
                              </span>
                              {stats.tipEarnings > 0 && (
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-blue-400">
                              $
                              {(
                                stats.stripeAvailableBalance || 0
                              ).toLocaleString()}
                            </p>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-purple-400 mt-1">
                              Videos awaiting customer review
                            </p>
                          </div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {stats.declinedThisMonth > 0 && (
                                <span className="text-xs text-red-400">
                                  {stats.declinedThisMonth} declined
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-purple-400 mt-1">
                              This month: $
                              {(stats.monthlyTipEarnings || 0).toLocaleString()}
                            </p>
                          </div>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-purple-400 mt-1">
                              {allOrders.filter((o) => o.tipAmount > 0).length}{" "}
                              with tips
                            </p>
                          </div>
                ...
                <Card className="bg-white/10 border-white/20 backdrop-blur-lg">
                  ...
                  <CardContent>
                    <p className="text-purple-200 mb-4">
                      Promote your celebrity profile on social media to attract
                      more fans and bookings!
                    </p>
                          ...
                          {request.approvalStatus?.toLowerCase() ===
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                              ...
                              <Button
                                onClick={() => {
                                  console.log(
                                    `🔴 Decline button clicked for request:`,
                                    request.id
                                  );
                                ...
                                <div className="bg-red-500/10 border border-red-500/20 p-3 rounded-lg">
                                  ...
                                  {order.revisionCount && (
                                    <p className="text-red-300 text-xs mt-2">
                                      Revision {order.revisionCount} of 2
                                      requested
                                    </p>
                                  )}
                                </div>
                ...
                <Card className="bg-white/10 border-white/20 backdrop-blur-lg">
                  ...
                  <CardContent>
                    <div className="text-center py-12">
                      ...
                      <p className="text-purple-200 mb-6">
                        Manage your availability and schedule booking slots for
                        your fans.
                      </p>
                      <Button className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600">
                      ...
                      >
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            onChange={(e) =>
                              updateProfile("longBio", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              updateProfile("responseTime", value)
                            }
                          >
