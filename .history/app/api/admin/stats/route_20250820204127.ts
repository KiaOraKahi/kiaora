...
export async function GET(request: NextRequest) {
  try {
    ...
    const userGrowth = lastMonthUsers === 0 ? (totalUsers > 0 ? 100 : 0) : ((totalUsers - lastMonthUsers) / lastMonthUsers) * 100
    const celebrityGrowth = lastMonthCelebrities === 0 ? (totalCelebrities > 0 ? 100 : 0) : ((totalCelebrities - lastMonthCelebrities) / lastMonthCelebrities) * 100
    const orderGrowth = lastMonthOrders === 0 ? (totalOrders > 0 ? 100 : 0) : ((totalOrders - lastMonthOrders) / lastMonthOrders) * 100
    const revenueGrowth = lastMonthRevenueSum === 0 ? (currentRevenue > 0 ? 100 : 0) : ((currentRevenue - lastMonthRevenueSum) / lastMonthRevenueSum) * 100
