...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Allow accepting bookings with PENDING or SUCCEEDED payment status
    if (action === "accept" && !["PENDING", "SUCCEEDED"].includes(booking.order?.paymentStatus || "")) {
      console.log("❌ Cannot accept booking with payment status:", booking.order?.paymentStatus)
      return NextResponse.json(
        {
          error: "Cannot accept booking. Payment status is not valid for acceptance.",
        },
        { status: 400 },
      )
    }
