    ...
    switch (action) {
      case "create": {
        ...
        if (celebrity.stripeConnectAccountId) {
          console.log("⚠️ Stripe Connect account already exists:", celebrity.stripeConnectAccountId)
        ...
        try {
          ...
          if (errorMessage.includes("signed up for Connect")) {
            console.log("❌ Stripe Connect not enabled error detected")
            return NextResponse.json({ 
              error: "Stripe Connect not enabled",
              details: "Your Stripe account needs to be enabled for Connect. Please enable Stripe Connect in your Stripe Dashboard first.",
              requiresSetup: true
            }, { status: 400 })
          }
