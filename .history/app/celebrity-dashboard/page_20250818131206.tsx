...
import CelebrityPayoutDashboard from "@/components/celebrity-payout-dashboard"
...
export default function CelebrityDashboard() {
  ...
  const [processingBooking, setProcessingBooking] = useState<string | null>(null) // Track which booking is being processed
  const [bookingActionSuccess, setBookingActionSuccess] = useState<string | null>(null) // Success message for booking actions
  ...
  // Debug orderFilter changes
  useEffect(() => {
    console.log('🔍 Order filter changed:', { orderFilter, activeTab })
  ...
  const handleBookingAction = async (requestId: string, action: "accept" | "decline") => {
    ...
    if (processingBooking) {
      console.log(`⏳ Already processing booking: ${processingBooking}`)
      return
    }
    
    try {
      setProcessingBooking(requestId)
      ...
      setBookingRequests((prev) =>
        prev.filter((request) => request.id !== requestId) // Remove the processed request
      )
      ...
      setStats((prev) => {
        ...
        return {
          ...
          ...(action === "accept" && { confirmedBookings: prev.confirmedBookings + 1 }),
          ...(action === "decline" && { declinedThisMonth: prev.declinedThisMonth + 1 }),
        }
      })

      // Update all orders without full refresh
      setAllOrders((prev) =>
        prev.map((order) =>
          order.id === requestId 
      ...
      setBookingActionSuccess(`Booking ${action === "accept" ? "accepted" : "declined"} successfully!`)
      setTimeout(() => setBookingActionSuccess(null), 3000)
    } catch (error) {
  ...
  const getFilteredOrders = () => {
    console.log('🔍 Filtering orders:', { orderFilter, totalOrders: allOrders.length, ordersLoading })
    
    // If still loading, return empty array
    if (ordersLoading) {
      console.log('⏳ Still loading orders, returning empty array')
      return []
    }
    ...
    if (orderFilter === "pending_approval") {
      const filtered = allOrders.filter((order) => order.approvalStatus?.toLowerCase() === "pending_approval")
      console.log('📋 Pending approval filter:', { 
        total: allOrders.length, 
        filtered: filtered.length,
        orders: allOrders.map(o => ({ id: o.id, approvalStatus: o.approvalStatus }))
      })
      return filtered
    }
    if (orderFilter === "approved") {
      const filtered = allOrders.filter((order) => order.approvalStatus?.toLowerCase() === "approved")
      console.log('📋 Approved filter:', { total: allOrders.length, filtered: filtered.length })
      return filtered
    }
    if (orderFilter === "declined") {
      const filtered = allOrders.filter((order) => order.approvalStatus?.toLowerCase() === "declined")
  ...
  // 🔥 NEW: Get orders that need revision (customer declined the video and revision count < 2)
  const getRevisionRequests = () => {
  ...
  // 🔥 NEW: Get declined booking requests (celebrity declined the booking)
  const getDeclinedBookingRequests = () => {
    return allOrders.filter((order) => 
      order.status === "cancelled" && 
      order.approvalStatus === null // No approval status means it was declined before video upload
    )
  }
  
  console.log("Revision requests (customer declined video):", getRevisionRequests());
  console.log("Declined booking requests (celebrity declined booking):", getDeclinedBookingRequests());
        ...
        <div className="container mx-auto px-4 pt-8 pb-12">
          ...
          {bookingActionSuccess && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="mb-6 p-4 bg-green-500/20 border border-green-500/30 rounded-lg backdrop-blur-lg"
            >
              <div className="flex items-center gap-3">
                <CheckCircle className="w-5 h-5 text-green-400" />
                <p className="text-green-200 font-medium">{bookingActionSuccess}</p>
              </div>
            </motion.div>
          )}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
               ...
               {getRevisionRequests().length > 0 && (
                 <Card className="bg-gradient-to-r from-red-500/20 to-orange-500/20 border-red-500/30 backdrop-blur-lg">
                   <CardHeader>
                     <CardTitle className="text-white flex items-center gap-2">
                       <AlertTriangle className="w-5 h-5 text-red-400" />🚨 Urgent: Video Revision Requests (
                       {getRevisionRequests().length})
                     </CardTitle>
                   </CardHeader>
                   <CardContent className="space-y-4">
                     <p className="text-red-200 text-sm mb-4">
                       These customers declined your videos and requested changes. Please upload revised videos as soon as
                       possible.
                     </p>
                    {getRevisionRequests()
                      .slice(0, 3)
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                console.log('🔍 Details button clicked, setting filter to declined')
                                setActiveTab("orders")
                            ...
                            >
                              ...
                              Details
                                         ...
                                         {getRevisionRequests().length > 3 && (
                       <Button
                         variant="outline"
                         className="w-full bg-red-500/10 border-red-500/30 text-red-300 hover:bg-red-500/20"
                         onClick={() => {
                           console.log('🔍 View All button clicked, setting filter to declined')
                           setActiveTab("orders")
                           setOrderFilter("declined")
                         }}
                       >
                         View All {getRevisionRequests().length} Video Revision Requests
                       </Button>
                     )}
                  </CardContent>
                </Card>
                          ...
                          {request.status === "pending" && (
                            <div className="flex gap-3">
                              ...
                              >
                                ...
                                {processingBooking === request.id ? "Processing..." : "Accept Request"}
                              </Button>
                              <Button
                                onClick={(e) => {
                                  e.preventDefault()
                              ...
                              >
                                ...
                                {processingBooking === request.id ? "Processing..." : "Decline"}
                              </Button>
                            </div>
                          )}
                        </motion.div>
