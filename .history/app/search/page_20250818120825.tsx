...
export default function SearchPage() {
  ...
  const filteredResults = useMemo(() => {
    console.log('🔍 Filtering celebrities:', {
      totalCelebrities: celebrities.length,
      searchTerm,
      celebrities: celebrities.map(c => ({ name: c.name, category: c.category }))
    })
    ...
    const results = celebrities.filter((celebrity) => {
      ...
      const categoryMatch = celebrity.category.toLowerCase().includes(searchLower)
      const bioMatch = celebrity.bio.toLowerCase().includes(searchLower)
      const tagsMatch = celebrity.tags.some((tag) => tag.toLowerCase().includes(searchLower))
      
      const matchesSearch = nameMatch || categoryMatch || bioMatch || tagsMatch
      
      if (searchTerm && celebrity.name === 'Emma Stone') {
        console.log('🔍 Emma Stone search debug:', {
          searchTerm,
          searchLower,
          nameMatch,
          categoryMatch,
            ...
            <div className="flex-1">
              {loading ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
                  ...
                  <p className="text-purple-200">Please wait while we fetch the latest data</p>
                </motion.div>
