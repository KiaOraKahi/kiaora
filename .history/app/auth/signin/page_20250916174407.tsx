  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Note: Role-based redirection should be handled by the session callback
        // This is a fallback for immediate redirection
        router.push("/")
      }
    } catch (error) {
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 hover:text-white disabled:opacity-50 text-white"
            >
