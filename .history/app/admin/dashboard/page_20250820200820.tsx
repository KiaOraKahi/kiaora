...
export default function AdminDashboard() {
  ...
  // All users for user management (separate from recent users in overview)
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch("/api/admin/users?limit=100") // Get more users for management
      if (response.ok) {
        const data = await response.json()
        // Format the users data to match the expected structure
        const formattedUsers = data.users.map((user: any) => ({
          ...user,
          joined: user.joined ? formatTimeAgo(new Date(user.joined)) : "Unknown"
        }))
    ...
    if (confirm(`Are you sure you want to delete ${user.name}?`)) {
      try {
        ...
        if (response.ok) {
          ...
          fetchUsers() // This will refresh allUsers
        } else {
  ...
  const handleAddUser = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNewUser({ name: "", email: "", role: "FAN", password: "" })
        fetchUsers()
      } else {
        const error = await response.json()
  ...
  const handleUpdateBookingStatus = async (bookingId: string, newStatus: string) => {
    try {
      ...
      if (response.ok) {
        ...
        fetchAdminData() // Refresh booking data
      } else {
        const error = await response.json()
  ...
  const handleSaveFinancialSettings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        toast.error(error.message || "Failed to save financial settings")
      }
    } catch (error) {
            ...
            <TabsContent value="overview" className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card className="bg-white/5 border-white/10 backdrop-blur-xl">
                  ...
                  <CardContent>
                                         <div className="text-2xl font-bold text-white">${dataLoading ? "..." : new Intl.NumberFormat('en-US').format(stats.totalRevenue)}</div>
                    <div className="flex items-center text-xs text-green-400">
                      <ArrowUpRight className="w-3 h-3 mr-1" />
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setFinancialSettings({...financialSettings, platformFee: Number(e.target.value)})}
                        className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setFinancialSettings({...financialSettings, minimumPayout: Number(e.target.value)})}
                        className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500"
                      />
                    </div>
                    <div className="space-y-2">
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  <option value="FAN" style={{backgroundColor: '#1a1a1a', color: 'white'}}>Fan</option>
                  <option value="CELEBRITY" style={{backgroundColor: '#1a1a1a', color: 'white'}}>Celebrity</option>
                  <option value="ADMIN" style={{backgroundColor: '#1a1a1a', color: 'white'}}>Admin</option>
                </select>
              </div>
              <div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <Badge 
                    variant={selectedUser.role === "CELEBRITY" ? "default" : "secondary"}
                    className={selectedUser.role === "CELEBRITY" ? "bg-yellow-500/20 text-yellow-300" : "bg-purple-500/20 text-purple-300"}
                  >
                    {selectedUser.role}
                  </Badge>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{selectedUser.joined}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/60">Status:</span>
                  <Badge 
                    variant={selectedUser.status === "active" ? "default" : "secondary"}
                    className={selectedUser.status === "active" ? "bg-green-500/20 text-green-300" : "bg-orange-500/20 text-orange-300"}
                  >
                    {selectedUser.status}
                  </Badge>
                </div>
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  <span className="text-white/60">Celebrity:</span>
                  <span className="text-white">{selectedBooking.celebrity}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge 
                    ...
                    className={
                      ...
                      selectedBooking.status === "COMPLETED" ? "bg-blue-500/20 text-blue-300" :
                      "bg-red-500/20 text-red-300"
                    }
                  >
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="CONFIRMED" style={{backgroundColor: '#1a1a1a', color: 'white'}}>Confirmed</option>
              ...
              <div className="space-y-2">
                <p className="text-sm text-white/60">Customer: {selectedBooking.customer}</p>
                <p className="text-sm text-white/60">Celebrity: {selectedBooking.celebrity}</p>
                <p className="text-sm text-white/60">Amount: ${selectedBooking.amount}</p>
              </div>
              <div className="flex space-x-3">
                <Button
