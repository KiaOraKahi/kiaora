    ...
    try {
      ...
      const response = await fetch("/api/stripe/connect", {
       ...
       if (response.ok && data.onboardingUrl) {
         ...
         console.log("✅ Redirecting to onboarding URL:", data.onboardingUrl)
         window.location.href = data.onboardingUrl
       } else {
         console.log("❌ API Error:", data)
         // Handle specific Stripe Connect not enabled error
         if (data.error === "Stripe Connect not enabled") {
           setError("Stripe Connect needs to be enabled. Please enable Stripe Connect in your Stripe Dashboard first.")
         } else {
          ...
          {error && (
            <Alert className="bg-red-500/20 border-red-500/30">
              ...
              <AlertDescription className="text-red-200">
                {error}
                {error.includes("Stripe Connect needs to be enabled") && (
                  <div className="mt-2">
                    <a 
                    ...
                    >
                      Click here to enable Stripe Connect →
                    </a>
                  </div>
                )}
              </AlertDescription>
