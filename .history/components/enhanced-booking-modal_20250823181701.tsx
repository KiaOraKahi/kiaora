...
export default function EnhancedBookingModal({ celebrity, selectedService, isOpen, onClose }: BookingModalProps) {
  ...
  const calculateTotal = () => {
    ...
    const basePrice = selectedService.startingPrice || selectedService.basePrice || 0
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      console.log("💰 Base price:", selectedService?.startingPrice || selectedService?.basePrice)
      ...
      if (totalAmount <= 0) {
        throw new Error("Invalid total amount. Please select a service and try again.")
      }
      ...
      if (response.ok) {
        ...
        console.log("🔑 Setting client secret:", data.clientSecret ? "EXISTS" : "MISSING")
