  ...
  const fetchPayoutData = async () => {
    try {
      ...
      const mockPayoutData: PayoutData = {
        ...
        stripeBalance: {
          available: [{ amount: 450000, currency: "nzd" }],
          pending: [{ amount: 200000, currency: "ror nzd" }],
        },
        recentPayouts: [],
        pendingOrders: []
      }
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">
                  {payoutData?.stripeBalance?.available && payoutData.stripeBalance.available.length > 0 
                    ? formatCurrency(payoutData.stripeBalance.available.reduce((sum, bal) => sum + (bal.amount / 100), 0))
                    : "$0.00"
                  }
                </p>
