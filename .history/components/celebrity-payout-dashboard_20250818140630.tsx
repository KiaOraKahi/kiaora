...
export default function CelebrityPayoutDashboard() {
  ...
  const fetchPayoutData = async () => {
    ...
    try {
      // Fetch real tip data from the database
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.setFontSize(12)
      const summaryData = [
        { label: 'Total Earnings', value: formatCurrency(payoutData.summary.totalEarnings), color: greenColor },
        { label: 'Pending Earnings', value: formatCurrency(payoutData.summary.pendingEarnings), color: yellowColor },
        { label: 'Total Tips', value: tipData ? formatCurrency(tipData.summary.totalTips) : '$0.00', color: pinkColor },
        { label: 'Platform Fees', value: formatCurrency(payoutData.summary.totalPlatformFees), color: blueColor },
      ]
      ...
      const maxValue = Math.max(
        ...
        tipData?.summary.totalTips || 0,
        payoutData.summary.totalPlatformFees
      )
      ...
      summaryData.forEach((item, index) => {
        ...
        const barHeight = (item.value.replace(/[$,]/g, '') * 100 / maxValue) * (chartHeight - 20) / 100
      ...
      yPos += 10
      if (payoutData.recentPayouts.length > 0) {
        // Table headers
        doc.setFontSize(10)
        doc.setTextColor(100, 100, 100)
        ...
        doc.line(20, yPos, 190, yPos)
                 ...
                 // Table data
         payoutData.recentPayouts.slice(0, 5).forEach((payout) => {
           yPos += 8
           doc.setFontSize(9)
           doc.setTextColor(0, 0, 0)
           doc.text(payout.order.orderNumber, 20, yPos)
           doc.setTextColor(...greenColor)
           doc.text(formatCurrency(payout.amount), 80, yPos)
           doc.setTextColor(payout.status === 'PAID' ? greenColor[0], greenColor[1], greenColor[2] : yellowColor[0], yellowColor[1], yellowColor[2])
           doc.text(payout.status, 130, yPos)
           doc.setTextColor(100, 100, 100)
           doc.text(format(new Date(payout.createdAt), "MMM d"), 170, yPos)
         })
      } else {
        yPos += 10
        doc.setFontSize(10)
