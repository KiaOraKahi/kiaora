...
export const createConnectAccount = async (celebrityData: {
  ...
  try {
    ...
    // 🔥 KEY FIX: For New Zealand accounts, we'll handle service agreement differently
    if (isNewZealand) {
      console.log("✅ New Zealand account - will handle service agreement during onboarding")
    }
    ...
    console.error("🔍 Stripe error details:", {
      ...
      decline_code: error instanceof Stripe.errors.StripeError ? error.decline_code : undefined
    })
...
export const updateExistingNZAccount = async (accountId: string) => {
  try {
    console.log("🔄 Updating existing NZ account:", accountId)
    
    // For now, we'll just return the account without updating service agreement
