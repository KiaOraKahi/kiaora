...
async function setupProductionDatabase() {
  ...
  try {
    ...
    try {
      ...
      const fs = require('fs')
      const originalEnvContent = fs.existsSync('.env') ? fs.readFileSync('.env', 'utf8') : ''
      
      // Create temporary .env with production database
      const tempEnvContent = `DATABASE_URL="${productionDatabaseUrl}"\n`
      fs.writeFileSync('.env.temp', tempEnvContent)
      
      // Use the temporary .env for schema push
