  ...
  try {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        userId: fanUser.id,
        totalAmount: celebrity.pricePersonal,
        celebrityAmount: celebrity.pricePersonal * 0.8, // 80% to celebrity
        platformFee: celebrity.pricePersonal * 0.2, // 20% platform fee
    ...
    await prisma.order.update({
      where: { id: order.id },
      data: {
        totalAmount: celebrity.pricePersonal + tip.amount,
        celebrityAmount: (celebrity.pricePersonal + tip.amount) * 0.8,
        platformFee: (celebrity.pricePersonal + tip.amount) * 0.2,
      }
    });
    ...
    console.log(`  - Base Price: $${celebrity.pricePersonal}`);
    console.log(`  - Tip: $${tip.amount}`);
    console.log(`  - Total: $${celebrity.pricePersonal + tip.amount}`);
