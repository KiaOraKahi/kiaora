...
async function createCelebrityProfiles() {
  try {
    console.log('🌟 Creating celebrity profiles for existing users...\n');

    // Get all users with CELEBRITY role who don't have celebrity profiles
    ...
    console.log(`Found ${celebrityUsers.length} users without celebrity profiles`);
    ...
    for (const user of celebrityUsers) {
      try {
        // Find matching celebrity data
        const celebData = celebrityData.find(data => data.name === user.name);
        
        if (!celebData) {
          console.log(`⚠️  No matching data found for ${user.name}, using default values`);
          continue;
        }
        ...
        // Update user image if it's missing
        if (!user.image) {
          await prisma.user.update({
            where: { id: user.id },
            data: { image: celebData.image }
          });
        }

        console.log(`✅ Created celebrity profile for: ${user.name} (${celebData.category})`);
        ...
        console.error(`❌ Error creating profile for ${user.name}:`, error.message);
      }
    }

    console.log('🎉 Finished creating celebrity profiles!');
    console.log('📊 All celebrities should now appear on both homepage and celebrities page.');
